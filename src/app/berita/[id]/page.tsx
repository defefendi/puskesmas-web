...
export default async function DetailBerita({ params }: { params: { id: string } }) {
  let item = null;
  let recentList: any[] = [];
  
  try {
    item = await databases.getDocument('puskesmaslenteng_db', 'pengumuman', params.id);
    const recentRes = await databases.listDocuments('puskesmaslenteng_db', 'pengumuman', [
    ...
    return (
      <main className="min-h-screen flex items-center justify-center bg-[var(--cream)]">
        <div className="text-center">
          ...
          <Link href="/berita" className="text-[var(--green-mid)] hover:underline">Kembali ke Daftar Berita</Link>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--cream)] flex flex-col">
      ...
      <div className="flex-1 w-full max-w-[800px] mx-auto px-5 py-12">
        <Link href="/berita" className="text-[var(--green-mid)] hover:underline mb-8 inline-block text-sm font-bold">
        ...
        <article className="bg-white rounded-[24px] overflow-hidden shadow-sm border border-gray-100 mb-12">
           ...
           <div className="p-6 md:p-10">
              <span className="text-[12px] bg-[#e6f4ea] text-[var(--green-deep)] font-bold px-3 py-1 rounded-md mb-4 inline-block">Berita & Kegiatan</span>
        ...
        {recentList.length > 0 && (
          <div className="bg-white p-6 rounded-[24px] shadow-sm border border-gray-100">
             <h3 className="text-lg font-bold text-[var(--ink)] mb-4 border-b border-gray-100 pb-3">Baca Juga Berita Lainnya</h3>
             <div className="flex flex-col gap-4">
               {recentList.map((recent: any, idx: number) => (
                 <Link href={`/berita/${recent.$id}`} key={idx} className="group flex gap-4 items-center">
                   {recent.fotoUrl ? (
